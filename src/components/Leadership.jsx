  ...
  const content = (
    ...
    >
      <div className="beyond-card-topline">
        <span className="beyond-card-number">{String(index + 1).padStart(2, '0')} <i>/ 04</i></span>
      ...
      <div className="beyond-card-body">
        <div className="beyond-card-copy">
          ...
          {item.link && <a href={item.link} target="_blank" rel="noopener noreferrer" className="beyond-card-link">View Certificate <span aria-hidden="true">&#8599;</span></a>}
        </div>
        <div className={`beyond-card-motif ${item.image ? 'beyond-card-motif-image' : ''}`} aria-hidden="true">
          {item.image ? (
  ...
  return (
    <section id="beyond-code" className={`beyond-code-section ${reducedMotion ? 'beyond-code-reduced' : ''}`}>
      <div className="beyond-code-inner">
        <header data-aos="fade-up" className="beyond-code-header text-center">
          <h2 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tight mb-4 uppercase">
        ...
        <div ref={stageRef} className="beyond-stack-stage">
          <div className="beyond-stack-sticky">
            <div className="beyond-progress" aria-label={`Journey progress ${activeIndex + 1} of ${leadershipList.length}`}>
              <span>{String(activeIndex + 1).padStart(2, '0')}</span><i>/</i><span>04</span>
              <span className="beyond-progress-label">CERTIFICATIONS</span>
            </div>
