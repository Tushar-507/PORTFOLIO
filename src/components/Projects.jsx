  ...
  <article data-aos="fade-up" data-aos-delay={index * 120} className="group flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-slate-200 bg-white shadow-[0_14px_40px_rgba(32,38,43,0.07)] transition-all duration-500 hover:-translate-y-2 hover:border-amber-700/30 hover:shadow-[0_24px_55px_rgba(32,38,43,0.14)]">
    <div className="relative aspect-[16/10] overflow-hidden bg-slate-100 flex items-center justify-center">
      {project.image ? (
        ...
        <span className="text-6xl font-light text-slate-300">+</span>
      )}
    ...
    <div className="flex flex-1 flex-col p-6 md:p-7">
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        {project.links.demo && <a href={project.links.demo} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-slate-900 px-4 py-2.5 text-xs font-bold text-white transition hover:bg-amber-700"><ExternalLinkIcon />View Project</a>}
        {project.links.demo && <a href={project.links.demo} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-4 py-2.5 text-xs font-bold text-slate-700 transition hover:border-slate-900 hover:text-slate-900">Live Demo<ExternalLinkIcon /></a>}
      </div>
    </div>
  </article>
);
